                        ...
                        @foreach ($this->user->tokens->sortBy('name') as $token)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                        <button class="ml-6 text-sm text-gray-400 underline cursor-pointer dark:text-gray-300 focus:outline-none" wire:click="manageApiTokenPermissions({{ $token->id }})">
                                            {{ __('Permissions') }}
                                        </button>
                                    @endif

                                    <button class="ml-6 text-sm text-red-500 cursor-pointer dark:text-red-400 focus:outline-none" wire:click="confirmApiTokenDeletion({{ $token->id }})">
                                        {{ __('Delete') }}
                                    </button>
                                </div>
                            </div>
